...
const MarkdownIt = require('markdown-it')//引用進來時預設為類別
const mdit = new MarkdownIt({ // 將MarkdownIt變成物件//創建 markdown 的轉換物件！將以下功能開啟
  html: true,//設定可以把HTML寫入
  linkify: true,//將網址以超連結方式顯示
  typographer: true
})
...
.get('/blog/:file', async (ctx) => { // 處理 GET 顯示檔案的請求
  ...
  switch (op) {
    case 'view': // 顯示檔案
      if (file.indexOf('.') >= 0) { // 第一種情形：有指定副檔名(含有".")，那麼直接傳回該檔案串流。
        ctx.type = ext // 設定傳回型態為《副檔名》對應的型態。
        ctx.body = fs.createReadStream(fpath) // 直接傳回該檔案串流
      } else { // 第二種情形：沒有副檔名，那麼預設是要呈現 .md 檔案
        let md = await fs.promises.readFile(fpath+'.md', 'utf8') // 等待讀取該 .md 檔案完畢再執行下一步(因為傳統readFile不是這樣寫且複雜,後來多加promises來支援這種較容易的寫法)
        ctx.body = mdRender(md, file) // Render將 markdown 轉為 HTML 傳回
      }
      break
  }
})
...
function layout (path, body) { // layout=>套用 HTML 樣板 (有 css 與 header 區塊)
  let html = `
<html>
<head>
